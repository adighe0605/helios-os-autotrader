...
export function mockDecision(symbol: string): TradeDecision {
  ...
  const score = Math.round((0.55 + r() * 0.4) * 100);
  const confidenceScore = Math.round((0.6 + r() * 0.3) * 100);
  return {
    symbol,
    verdict,
    ...
    score,
    recommendation: score >= 80 ? "⭐⭐⭐⭐ Buy" : score >= 60 ? "⭐⭐⭐ Hold / Watch" : "⭐⭐ Sell",
    positionSize: score >= 80 ? "5%" : "2%",
    risk: "Medium",
    agreement: "Medium",
    bullishSignals: ["Technical trend above VWAP", "Positive momentum"],
    bearishSignals: ["Macro uncertainty"],
    conflictingSignals: ["Technical bullish + macro mixed"],
    supportingEvidence: ["Price above 20D SMA", "Relative volume elevated"],
    nextReview: "15 minutes",
    risk_vetoed: false,
    risk_veto_reason: null,
    agent_contributions: [],
    section_scores: {
      "market-intelligence": score,
      "technical-analysis": score,
      "news-intelligence": score,
    ...
    section_data_status: {
      ...
      "news-intelligence": false,
      "social-sentiment": false,
      "options-flow": false,
